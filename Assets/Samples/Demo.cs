...
namespace NvgExample
{
    public class Demo : IDisposable
    {
        ...
        public Demo(Nvg nvg, DemoResources resources)
        {
            _nvg = nvg;

            for (uint i = 0; i < resources.Images.Length; i++)
            {
                _images[i] = _nvg.CreateImageMem(0, resources.Images[i].bytes);
                if (_images[i] == 0)
                {
                    Console.Error.WriteLine("Could not load image " + i);
                }
            }

            _fontIcons = _nvg.CreateFontMem("icons", resources.EntypoFont.bytes, 1);
            if (_fontIcons == -1)
            {
                Console.Error.WriteLine("Could not add font icons.");
            }
            _fontNormal = _nvg.CreateFontMem("sans", resources.RobotoRegularFont.bytes, 1);
            if (_fontIcons == -1)
            {
                Console.Error.WriteLine("Could not add font regular.");
            }
            _fontBold = _nvg.CreateFontMem("sans-bold", resources.RobotoBoldFont.bytes, 1);
            if (_fontIcons == -1)
            {
                Console.Error.WriteLine("Could not add font bold.");
            }
            _fontEmoji = _nvg.CreateFontMem("emoji", resources.NotoEmojiRegularFont.bytes, 1);
            if (_fontIcons == -1)
            {
